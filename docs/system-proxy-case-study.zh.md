...
**如果将来某个版本把这条路堵死** —— 比如给 `ProcessPlatformOptions` 加上
`auto_route: true` 的前置条件 —— 备选是让内核**跑在登录用户的会话里**，而不是作为服务
运行（由用户直接启动 `sing-box run -c config.json`，而不是由 SCM 拉起）。
此时 `set_system_proxy` 会自己写进正确的 hive —— 因为进程本身就是那个用户。
代价是：TUN 模式需要显式提权。上面的调用链来自 1.14.1，升级内核后需重新核对。
